...
import com.mppay.service.entity.TradeRefundCheckWeixin;
import com.mppay.service.entity.TradeRefundWeixin;
import com.mppay.service.service.ITradeRefundCheckWeixinService;
 ...
 *
 */
@Service("weixinRefundCheckCenterHandler")
@Slf4j
public class WeixinRefundCheckCenterHandlerImpl extends BaseCheckCenterHandlerImpl implements CheckCenterHandler {
	@Autowired
	private ITradeRefundCheckWeixinService tradeRefundCheckWeixinService;
	...
	public void importData(Long batchId) throws Exception {
		...
		String line = null;
		String[] data = null;
		TradeRefundCheckWeixin tradeRefundCheckWeixin = null;
		int rowNum = 0;
		try {
			for (String fileName : fileNameArray) {
				...
				while ((line = br.readLine()) != null) {
					...
					tradeRefundCheckWeixin = new TradeRefundCheckWeixin();
					tradeRefundCheckWeixin.setAccountDate(checkControl.getAccountDate());
					tradeRefundCheckWeixin.setRouteCode(checkControl.getRouteCode());
					tradeRefundCheckWeixin.setBatchId(batchId);
					tradeRefundCheckWeixin.setCheckStatus("0");
					tradeRefundCheckWeixin.setCheckDate(DateTimeUtil.date10());
					tradeRefundCheckWeixin.setCheckTime(DateTimeUtil.time8());
					tradeRefundCheckWeixin.setBankMercId(data[2]);
					tradeRefundCheckWeixin.setBankTradeNo(data[5]);
					tradeRefundCheckWeixin.setOutTradeNo(data[6]);
					tradeRefundCheckWeixin.setTradeType(data[8]);
					tradeRefundCheckWeixin.setFundBank(data[10]);
					tradeRefundCheckWeixin.setBankRefundNo(data[16]);
					tradeRefundCheckWeixin.setOutRefundNo(data[17]);
					tradeRefundCheckWeixin.setPrice(new BigDecimal(data[18]));
					String bankReturnStatus = data[21];
					if (ConstEC.SUCCESS.equals(bankReturnStatus.trim())) {
						bankReturnStatus = "S";
					} else if (ConstEC.FAIL.equals(bankReturnStatus.trim())) {
						bankReturnStatus = "F";
					}
					tradeRefundCheckWeixin.setBankReturnStatus(bankReturnStatus);
					
					tradeRefundCheckWeixinService.save(tradeRefundCheckWeixin);
				}
	...
	public void checkData(Long batchId) throws Exception {
		...
		// 查询所有未对账的退款数据
		List<TradeRefundCheckWeixin> lists = tradeRefundCheckWeixinService.list(new QueryWrapper<TradeRefundCheckWeixin>()
...
//		}
		...
		TradeRefundWeixin tradeRefundWeixin = null;
		TradeRefund tradeRefund = null;
		String orderStatus = null;
		
		for (TradeRefundCheckWeixin obj : lists) {
			flag = false;
			orderStatus = null;
			...
			// 查询退款流水，对账状态0
			tradeRefundWeixin = tradeRefundWeixinService.getOne(new QueryWrapper<TradeRefundWeixin>()
					.eq("out_refund_no", obj.getOutRefundNo()).in("check_status", CheckStatus.WAITCHECK.getId(), CheckStatus.DOUBT.getId()));


			if (null != tradeRefundWeixin) {
				// 判断退款对账流水数据状态
				if (OrderStatus.SUCCESS.getId().equals(obj.getBankReturnStatus())) {
					// 比较金额是否相等
					if (tradeRefundWeixin.getApplyPrice().compareTo(obj.getPrice()) == 0) {
						checkStatus = "1";
						tatolSuccessPrice = tatolSuccessPrice.add(tradeRefundWeixin.getApplyPrice());
						tatolSuccessNum++;
						// 如果金额相等，判断充值流水
						if (OrderStatus.REFUND_WAIT.getId().equals(tradeRefundWeixin.getOrderStatus())) {
							orderStatus = "S";
						}
					} else {
						// 金额匹配不相同，金额差错
						checkStatus = "4";
						errorTatolPrice = errorTatolPrice.add(tradeRefundWeixin.getApplyPrice());
						errorTatolNum++;
					}
				} else {
					// 没有数据记录: 对方有，我方无
			...
			// 更新对账流水
			tradeRefundCheckWeixinService.updateById(obj);

			// 如果对账流水有数据，充值退款流水表没有数据记录，则不更新充值退款流水表
			if (!flag) {
				tradeRefundWeixin.setCheckDate(checkDate);
				tradeRefundWeixin.setCheckStatus(checkStatus);
				tradeRefundWeixin.setOrderStatus(orderStatus);
				tradeRefundWeixin.setActualPrice(obj.getPrice());
				tradeRefundWeixin.setBankRefundNo(obj.getBankRefundNo());
				// 更新充值退款流水
				tradeRefundWeixinService.updateById(tradeRefundWeixin);

				tradeRefund = tradeRefundService.getOne(new QueryWrapper<TradeRefund>()
						.eq("refund_no", tradeRefundWeixin.getRefundNo()));
		...
		tradeRefundWeixinService.update(new TradeRefundWeixin(), new UpdateWrapper<TradeRefundWeixin>()
		...
		tradeRefundWeixin = new TradeRefundWeixin();
		tradeRefundWeixin.setCheckStatus(CheckStatus.WAITCHECK.getId());
		tradeRefundWeixin.setOrderStatus(OrderStatus.SUCCESS.getId());
		tradeRefundWeixin.setRefundDate(checkControl.getAccountDate());
		// 统计交易流水状态为0的数据
		Map<String, Object> tradeRefundWeixinMap = tradeRefundWeixinService.statPrice(tradeRefundWeixin);
		// 统计存疑金额
		dubiousPrice = new BigDecimal(tradeRefundWeixinMap.get("total_price") == null ? "0" : tradeRefundWeixinMap.get("total_price").toString());
		dubiousNum = tradeRefundWeixinMap.get("total_price") == null ? 0 : Integer.parseInt(tradeRefundWeixinMap.get("total_price").toString());
		...
		// 查询对账状态为3、4的差错数据
		List<TradeRefundCheckWeixin> tradeRefundCheckWeixinList = tradeRefundCheckWeixinService.list(
				new QueryWrapper<TradeRefundCheckWeixin>().eq("check_date", checkDate)
				.in("check_status", "3", "4")
				);
				
		if (null != tradeRefundCheckWeixinList) {
			CheckError checkError = null;
			// 登记对方有我方无，金额差异数据
			for (TradeRefundCheckWeixin obj : tradeRefundCheckWeixinList) {
				tradeRefundWeixin = new TradeRefundWeixin();
				tradeRefundWeixin.setOutRefundNo(obj.getOutRefundNo());
				tradeRefundWeixin.setRouteCode(obj.getRouteCode());
				tradeRefundWeixin.setBankTradeNo(obj.getBankTradeNo());

				tradeRefundWeixin = tradeRefundWeixinService.getOne(
						new QueryWrapper<TradeRefundWeixin>().eq("out_refund_no", obj.getOutRefundNo())
						);
				...
				if (null != tradeRefundWeixin) {
					checkError.setOurPrice(tradeRefundWeixin.getPrice());
				}
		...
		// 查询对账状态为2的短款数据
		List<TradeRefundWeixin> tradeRefundWeixinList = tradeRefundWeixinService.list(
				new QueryWrapper<TradeRefundWeixin>().eq("check_date", checkDate)
				.eq("check_status", CheckStatus.SHORT.getId())
				);
		
		if (null != tradeRefundWeixinList) {
			CheckError checkError = null;
			// 登记短款差异数据
			for (TradeRefundWeixin obj : tradeRefundWeixinList) {
				checkError = new CheckError();
				checkError.setBatchId(batchId);
				checkError.setBankTradeNo(obj.getBankTradeNo());
				checkError.setRouteCode(obj.getRouteCode());
	...
	protected String sendMsg(String appid, String mercId, String key, CheckControl checkControl) throws Exception {
		...
		bankMap.put("bill_type", ConstEC.REFUND);
		...
		// 3、调用微信统一下单接口 发送请求报文
		byte[] responseData = HttpClientUtil.httpsRequestRetByteArray("https://api.mch.weixin.qq.com/pay/downloadbill", "POST",
				requestXml);
