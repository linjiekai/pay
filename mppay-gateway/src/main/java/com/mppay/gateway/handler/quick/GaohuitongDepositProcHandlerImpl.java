...
@Service("gaohuitongDepositProcHandler")
@Transactional(rollbackFor = Exception.class)
@Slf4j
public class GaohuitongDepositProcHandlerImpl extends BaseDepositProcHandlerImpl implements DepositProcHandler {
	...
	public void procWithdr(Map<String, Object> data) throws Exception {
		...
		//更新提现子订单为成功 。data里面有order_status=S
		boolean flag = withdrOrderGaohuitongService.update(withdrOrderGaohuitong, new UpdateWrapper<WithdrOrderGaohuitong>()
				.eq("out_trade_no", data.get("outTradeNo"))
				.eq("order_status", WithdrOrderStatus.BANK_WAIT.getId())
				);
		
		if (!flag) {
			log.error("支付订单号OutTradeNo[{}], 订单状态不等于[{}]", withdrOrderGaohuitong.getOutTradeNo(), OrderStatus.WAIT_PAY.getId());
			throw new BusiException("11101", ApplicationYmlUtil.get("11101"));
		}
